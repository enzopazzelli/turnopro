...
export function PerfilProfesional({ profesional, tenant, configPP = {}, slug }) {
  ...
  const colorPrimario = configPP.color_primario || "hsl(var(--primary))";
  const heroUrl = configPP.hero_url;
    ...
    <section
      ...
      style={
        heroUrl
          ...
          : { background: `linear-gradient(135deg, ${colorPrimario} 0%, color-mix(in srgb, ${colorPrimario} 70%, black) 100%)` }
      }
    >
      {/* Overlay for readability when using hero image */}
      {heroUrl && <div className="absolute inset-0 bg-black/50" />}

      {/* Decorative circles (only on gradient) */}
      {!heroUrl && (
        <>
      ...
      <div className="relative max-w-4xl mx-auto px-4 py-16 flex flex-col items-center text-center gap-5 text-white">
        ...
        {configPP.mensaje_bienvenida && (
          <p className="text-white/90 max-w-xl italic text-base">
            &ldquo;{configPP.mensaje_bienvenida}&rdquo;
          </p>
        )}

        {/* Social links */}
        {(configPP.instagram || configPP.facebook || configPP.whatsapp) && (
          <div className="flex items-center gap-3 mt-1">
            {configPP.instagram && (
              <a
                href={
                  configPP.instagram.startsWith("http")
                    ? configPP.instagram
                    : `https://instagram.com/${configPP.instagram.replace("@", "")}`
                }
                target="_blank"
            ...
            {configPP.facebook && (
              <a
                href={
                  configPP.facebook.startsWith("http")
                    ? configPP.facebook
                    : `https://facebook.com/${configPP.facebook}`
                }
                target="_blank"
                rel="noopener noreferrer"
                className="p-2.5 rounded-full bg-white/15 hover:bg-white/25 transition-colors"
            ...
            {configPP.whatsapp && (
              <a
                href={`https://wa.me/${configPP.whatsapp.replace(/[\s\-+()]/g, "")}`}
                target="_blank"
                rel="noopener noreferrer"
                className="p-2.5 rounded-full bg-white/15 hover:bg-white/25 transition-colors"
                aria-label="WhatsApp"
